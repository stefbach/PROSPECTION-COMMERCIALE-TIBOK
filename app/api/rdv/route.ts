...
import { Calendar, Clock, Phone, MapPin, User, AlertCircle, CheckCircle } from 'lucide-react'
...
// Structure RDV alignée avec l'API
interface RendezVous {
  id: number
  prospect_id: number
  prospect_nom?: string
  prospect?: Prospect // Pour la compatibilité
  commercial: string
  titre: string
  date_time: string
  duree_min: number
...
export default function PlanningAdvancedSection() {
  ...
  // MODIFICATION : Charger les RDV depuis l'API
  async function loadRdvs() {
    try {
      ...
      // Enrichir les RDV avec les données prospects si nécessaire
      const enrichedRdvs = data.map((rdv: any) => {
        // Si le prospect n'est pas inclus, on essaie de le trouver
        if (!rdv.prospect && rdv.prospect_id) {
  ...
  // MODIFICATION : Créer un RDV via l'API
  async function addRdv(prospectId: number, prospect: Prospect, date: string, time: string, duree: number = 60, notes?: string) {
    try {
      const rdvData = {
        ...
        priorite: prospect.score >= 4 ? 'haute' as const : prospect.score >= 3 ? 'moyenne' as const : 'normale' as const,
      ...
      // Recharger les RDV pour avoir la liste à jour
      await loadRdvs()
  ...
  // MODIFICATION : Mettre à jour un RDV via l'API
  ...
  // MODIFICATION : Supprimer un RDV via l'API
  ...
  return (
    <div className="space-y-6">
          ...
          {filteredRdvs.length === 0 ? (
            ...
            <div className="space-y-3">
              ...
              {filteredRdvs.map((rdv, index) => {
                if (!rdv.prospect) {
                  // Si le prospect n'est pas chargé, on essaie de le trouver
                  rdv.prospect = prospects.find(p => p.id === rdv.prospect_id)
                }
      ...
      {/* Dialogs - Ajouter les dialogs de configuration ici */}
      <RdvDialog
...
// Dialog RDV simplifié et unifié
function RdvDialog({ 
  open, 
  onClose, 
  prospects, 
