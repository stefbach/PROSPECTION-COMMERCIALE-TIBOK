...
// Type Prospect simplifié si pas déjà défini
interface Prospect {
  id: number
  nom: string
  ville: string
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        <Button size="lg" className="gap-2" onClick={() => setShowFormDialog(true)}>
      ...
      {/* Formulaire dans un panneau latéral ou modal */}
      {showFormDialog && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <Card className="max-w-2xl w-full max-h-[90vh] overflow-y-auto">
            <CardHeader>
              <CardTitle>Planifier un nouveau rendez-vous</CardTitle>
            </CardHeader>
            <CardContent>
...
function RdvCard({ 
  ...
  if (viewMode === 'grid') {
    return (
      <Card className={`border-l-4 ${priorityColor} hover:shadow-lg transition-shadow cursor-pointer`} onClick={() => onViewDetails(rdv)}>
        <CardContent className="p-4">
  ...
  // Vue liste
  return (
    <Card className={`border-l-4 ${priorityColor} hover:shadow-md transition-shadow cursor-pointer`} onClick={() => onViewDetails(rdv)}>
      <CardContent className="p-4">
        <div className="flex items-center justify-between">
